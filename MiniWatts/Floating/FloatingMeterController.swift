...
///
/// iOS 16 compatibility: `ObservableObject` instead of the iOS 17 Observation
/// framework; the injected properties below are `@Published`.
final class FloatingMeterController: ObservableObject {
    ...
    @Published var layout: FloatingMeterLayout {
        didSet {
    ...
    private func renderLatest() {
        ...
        // iOS 16 compatibility: enqueue on the layer itself. The layer's own
        // `enqueue`/`flush`/`status` predate iOS 17's `sampleBufferRenderer` and
        // behave identically; they are deprecated on 17 but still work there.
