...
struct ThermalView: View {
    ...
    private var statePanel: some View {
        Panel("System thermal state", systemImage: "cpu",
              trailing: Text(verbatim: Formatting.duration(Date.now.timeIntervalSince(monitor.thermalStateSince)))) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .center, spacing: 12) {
                    Image(systemName: monitor.thermalState.symbol)
                ...
                severityMeter
                if snapshot.externalConnected, monitor.thermalState.isThrottling {
            ...
            GeometryReader { geometry in
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        ...
                        .frame(width: geometry.size.width * monitor.thermalState.severity)
                }
            }
            .frame(height: 6)
            HStack {
                ForEach([ProcessInfo.ThermalState.nominal, .fair, .serious, .critical],
                    ...
                    Text(state.title)
                        ...
                        .foregroundStyle(state == monitor.thermalState ? stateTint : Color.mwMuted)
                        .frame(maxWidth: .infinity)
                }
            }
        }
        .animation(.easeInOut(duration: 0.4), value: monitor.thermalState)
    }
