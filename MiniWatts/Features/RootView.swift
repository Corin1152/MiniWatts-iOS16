...
struct RootView: View {
    // iOS 16 compatibility: `@EnvironmentObject` instead of the Observation-based
    // `@Environment(PowerMonitor.self)`.
    @EnvironmentObject private var monitor: PowerMonitor
    @EnvironmentObject private var floatingMeter: FloatingMeterController
    ...
    var body: some View {
        ...
        // iOS 16 compatibility: the two-parameter `onChange(of:initial:)` is iOS
        // 17-only. The one-parameter form has no `initial`, so the first run the
        // original relied on is done explicitly in `onAppear`.
        .onAppear {
            monitor.start()
