...
extension View {
    /// `containerBackground(for: .widget)` is iOS 17-only. Below iOS 17 the
    /// modifier must be absent: the system does not require a container
    /// background there and the views already paint their own canvas.
    @ViewBuilder
    func mwWidgetBackground(family: WidgetFamily,
                            @ViewBuilder background: @escaping () -> some View) -> some View {
